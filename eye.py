...
class Eye:
    ...
    def update(self, image: np.ndarray, landmarks: list, offset: np.ndarray):
        ...
        if self.side == "right":
            ...
            self.pupil = landmarks[self.pupil_index]
    ...
    def getGaze(self, gaze_buffor, y_correction=0, x_correction=0):
        ...
        pupil_corrected = self.pupil

        vectors = region_corrected - center
        pupil = pupil_corrected
    ...
    def _process(self, image, region):
        ...
        self.minMax = [
            ...
            [max_x,(max_y-min_y)/2 + min_y],
            [(max_x - min_x)/2 + min_x,max_y],
            [(max_x - min_x)/2 + min_x,min_y],
        ]
